...
/*
    p.28 Tip
        @ : 어노테이션
            - 주석이나 해석으로 할수 있는데 주로 코드상에 추가적인 정보를 남겨두기 위해서 사용
            - 특정한 코드에 대해 추가적인 처리나 설정을 위해서 사용
            - 특정 클래스에 추가적인 외부 기능 부여할때 상속/구현체 보다 확장성이 좋다.

    - 다른 클래스에게 데이터 전달하는 방법
        - extends : 상속 [ 클래스설계도 연장 ]
        - implements : 구현 [ 인터페이스의 추상메소드 구현체 ]
            - 클래스 상속과 인터페이스 구현 한계..
                1. 클래스는 상속 1번 만 가능
                2. 인터페이스는 메모리 전달 불가능.. [ 일반 필드를 가질수 없다. 무조건 상수 필드 ]

        - @어노테이션
            1. 빌트인          @Override : JDK내 내장(기능)된 어노테이션
            2. 메타데이터       : 외부라이브러리 나 생성된 어노테이션 : 인터페이스나 클래스들을 확장해서 구조화된 데이터를 전달해주는
